$(document).ready(function () {
    document.getElementById("wdlc1").text = "A Schneider biztonsága";
    document.getElementById("wdlc2").text = "Elektromos biztonság";
    document.getElementById("wdlc3").text = "Ergonómia";
    document.getElementById("wdlc4").text = "Biztonságos indítás";
    document.getElementById("wdlc5").text = "Egyéb témák";
    document.getElementById("wdlc6").text = "COVID-19: Amit tudnod kell";
    document.getElementById("wdlc7").text = "A jólét kezelése a COVID-19 alatt";


    document.getElementById("wdlc11").text = "Alapvető értékeink";
    document.getElementById("wdlc12").text = "Az innovációs kör: az innováció és a változás három fázisa";
    document.getElementById("wdlc13").text = "Az SPS 40 alapelve";
    document.getElementById("wdlc14").text = "Cégünk története";
    document.getElementById("wdlc15").text = "EcoStruxure";


    document.getElementById("wdlc21").text = "Az ügyfél az első";
    document.getElementById("wdlc22").text = "Persona";


    document.getElementById("wdlc31").text = "Raktárkezelési tudatosság";
    document.getElementById("wdlc32").text = "Intelligens terjesztési központ";
    document.getElementById("wdlc33").text = "Fuvarozás";


    document.getElementById("wdlc41").text = "Lean gyártás";
    document.getElementById("wdlc42").text = "Short Interval Management";
    document.getElementById("wdlc43").text = "Intelligens gyár";
    document.getElementById("wdlc44").text = "Ellátásilánc-menedzsment";



    document.getElementById("wdlc51").text = "ETO folyamatok optimalizálása";



    document.getElementById("wdlc61").text = "Vevői elégedettség és minőség";
    document.getElementById("wdlc62").text = "Egészség és biztonság";
    document.getElementById("wdlc63").text = "Iparosítás";
    document.getElementById("wdlc64").text = "Logisztika";
    document.getElementById("wdlc65").text = "Gyártás";
    document.getElementById("wdlc66").text = "Összes";


    document.getElementById("wdlc71").text = "Együtt, fejlődünk!";
    document.getElementById("wdlc72").text = "Kommunikációs képesség";
    document.getElementById("wdlc73").text = "Változáskezelés";
    document.getElementById("wdlc74").text = "A boldogság titkai";
    document.getElementById("wdlc75").text = "Vállalkozói Kezdő készlet";


    document.getElementById("wdlc81").text = "goFluent";
    document.getElementById("wdlc82").text = "Tanuljon Angolul";
    document.getElementById("wdlc83").text = "Tanulj Spanyolul";
    document.getElementById("wdlc84").text = "Tanulj Franciául";



    document.getElementById("wdlc91").text = "Microsoft Office 365";
    document.getElementById("wdlc95").text = "Számítógépes ismeretek";
    document.getElementById("wdlc96").text = "Digitális képesség";
    document.getElementById("wdlc97").text = "A számítógép alapjai";



    document.getElementById("wdlc101").text = "My LearningLink";
    document.getElementById("wdlc102").text = "Katalógus böngészése";
    document.getElementById("wdlc103").text = "Schneider IQ";
    document.getElementById("wdlc104").text = "GSC Akadémia";
    document.getElementById("wdlc105").text = "Munkavállalói Fejlesztő Oldal";
    document.getElementById("wdlc106").text = "Tanulósarok más területekről";
    document.getElementById("wdlc107").text = "Elektromosság alapjai";
    document.getElementById("wdlc108").text = "Külső digitális képzések";

    
});