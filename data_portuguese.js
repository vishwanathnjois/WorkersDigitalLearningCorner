$(document).ready(function () {
    document.getElementById("wdlc1").text = "Segurança na Schneider Electric";
    document.getElementById("wdlc2").text = "Segurança Elétrica";
    document.getElementById("wdlc3").text = "Ergonomia";
    document.getElementById("wdlc4").text = "Início Seguro";
    document.getElementById("wdlc5").text = "Outros Tópicos";
    document.getElementById("wdlc6").text = "COVID 19 - O que você precisa saber";
    document.getElementById("wdlc7").text = "Gerenciando seu bem-estar durante COVID19";


    document.getElementById("wdlc11").text = "Valores fundamentais";
    document.getElementById("wdlc12").text = "O Círculo de Inovação: 3 fases de inovação e mudança";
    document.getElementById("wdlc13").text = "40 princípios de SPS";
    document.getElementById("wdlc14").text = "História da nossa empresa";
    document.getElementById("wdlc15").text = "EcoStruxure";


    document.getElementById("wdlc21").text = "Cliente Primeiro";
    document.getElementById("wdlc22").text = "Personas";


    document.getElementById("wdlc31").text = "Conhecimento de Gerenciamento de Almoxarifado";
    document.getElementById("wdlc32").text = "Centro de Distribuição Inteligente";
    document.getElementById("wdlc33").text = "Transportes";


    document.getElementById("wdlc41").text = "Manufatura Enxuta";
    document.getElementById("wdlc42").text = "Gerenciamento de Intervalo Curto";
    document.getElementById("wdlc43").text = "Fábrica Inteligente";
    document.getElementById("wdlc44").text = "Gerenciamento de Supply Chain";



    document.getElementById("wdlc51").text = "Otimize Processos ETO";



    document.getElementById("wdlc61").text = "Satisfação e qualidade do cliente";
    document.getElementById("wdlc62").text = "Saúde e Segurança";
    document.getElementById("wdlc63").text = "Industrialização";
    document.getElementById("wdlc64").text = "Logística";
    document.getElementById("wdlc65").text = "Manufatura";
    document.getElementById("wdlc66").text = "Tudo";


    document.getElementById("wdlc71").text = "Juntos, Crescemos!";
    document.getElementById("wdlc72").text = "Habilidades de Comunicação";
    document.getElementById("wdlc73").text = "Avante, a mudança, vingadores!";
    document.getElementById("wdlc74").text = "Segredos da Felicidade";
    document.getElementById("wdlc75").text = "Kit de Iniciação ao Empreendedor";


    document.getElementById("wdlc81").text = "goFluent";
    document.getElementById("wdlc82").text = "Aprender Inglês";
    document.getElementById("wdlc83").text = "Aprender Espanhol";
    document.getElementById("wdlc84").text = "Aprender Francês";



    document.getElementById("wdlc91").text = "Microsoft Office 365";
    document.getElementById("wdlc95").text = "Habilidades de informática";
    document.getElementById("wdlc96").text = "Capacidade Digital";
    document.getElementById("wdlc97").text = "Noções básicas de computador";



    document.getElementById("wdlc101").text = "My LearningLink";
    document.getElementById("wdlc102").text = "Pesquise no Catálogo";
    document.getElementById("wdlc103").text = "Schneider IQ";
    document.getElementById("wdlc104").text = "Academia GSC";
    document.getElementById("wdlc105").text = "Portal de Desenvolvimento do Colaborador";
    document.getElementById("wdlc106").text = "Ilha Digital Inter-Funcional";
    document.getElementById("wdlc107").text = "Teoria Básica em Elétrica";
    document.getElementById("wdlc108").text = "Treinamentos Digitais Externos";

    
});