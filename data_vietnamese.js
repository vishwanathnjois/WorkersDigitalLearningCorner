$(document).ready(function () {
    document.getElementById("wdlc1").text = "An toàn tại Schneider";
    document.getElementById("wdlc2").text = "An toàn điện";
    document.getElementById("wdlc3").text = "Công thái học";
 
    document.getElementById("wdlc5").text = "Các chủ đề khác";
    document.getElementById("wdlc6").text = "COVID-19: Những điều bạn cần biết";
    document.getElementById("wdlc7").text = "Quản lý sức khỏe của bạn trong COVID-19";


    document.getElementById("wdlc11").text = "Schneider Essentials";
    document.getElementById("wdlc12").text = "EcoStruxure";
    document.getElementById("wdlc13").text = "SSE";
    document.getElementById("wdlc14").text = "Lean Manufacturing";
    document.getElementById("wdlc15").text = "Quality";
    document.getElementById("wdlc16").text = "Logistics";
    document.getElementById("wdlc17").text = "Industrialization";
    document.getElementById("wdlc18").text = "Digital Acumen";
    document.getElementById("wdlc19").text = "STRIVE";
    document.getElementById("wdlc11D").text = "ETO";
		 document.getElementById("wdlc11E").text = "Video giới thiệu";


    document.getElementById("wdlc21").text = "Khách hang là trên hết";
    document.getElementById("wdlc22").text = "Cá nhân";


    document.getElementById("wdlc31").text = "Nhận thức về Quản lý Kho";
    document.getElementById("wdlc32").text = "Phân phối thông minh";
    document.getElementById("wdlc33").text = "Vận chuyển";


    document.getElementById("wdlc41").text = "Sản xuất tinh gọn";
    document.getElementById("wdlc42").text = "Quản lý theo giai đoạn";
    document.getElementById("wdlc43").text = "Nhà máy thông minh";
    document.getElementById("wdlc44").text = "Ngành công nghiệp";



    document.getElementById("wdlc51").text = "Tối ưu hóa Quá trình ETO";



    document.getElementById("wdlc61").text = "Sự hài lòng và chất lượng của khách hàng";
    document.getElementById("wdlc62").text = "sức khỏe và an toàn";
    document.getElementById("wdlc63").text = "Ngành công nghiệp";
    document.getElementById("wdlc64").text = "Logistics";
    document.getElementById("wdlc65").text = "Sản xuất";
    document.getElementById("wdlc66").text = "Tất cả";


    document.getElementById("wdlc71").text = "Cùng Nhau, Chúng Ta cùng phát triển";
    document.getElementById("wdlc72").text = "Giao tiếp & I";
    document.getElementById("wdlc73").text = "Thay đổi nhanh chóng";
    document.getElementById("wdlc74").text = "Những bí mật hạnh phúc";
    document.getElementById("wdlc75").text = "Bộ công cụ khởi động doanh nghiệp";


    document.getElementById("wdlc81").text = "goFluent";
    document.getElementById("wdlc82").text = "Học tiếng Anh";
    document.getElementById("wdlc83").text = "Học tiếng Tây Ban Nha";
    document.getElementById("wdlc84").text = "Học tiếng Pháp";



    document.getElementById("wdlc91").text = "Microsoft Office 365";
    document.getElementById("wdlc95").text = "Kỹ năng tin học";
    document.getElementById("wdlc96").text = "Khả năng kỹ thuật số";
    document.getElementById("wdlc97").text = "Thông tin cơ bản về máy tính: Máy tính là gì?";



    document.getElementById("wdlc101").text = "Liên kết học tập của tôi";
    document.getElementById("wdlc102").text = "Duyệt qua danh mục";
    document.getElementById("wdlc103").text = "Schneider IQ";
    document.getElementById("wdlc104").text = "Học viện GSC";
    document.getElementById("wdlc105").text = "Cổng thông tin phát triển nhân viên";
    document.getElementById("wdlc106").text = "Góc học tập về các bộ phận liên quan";
    document.getElementById("wdlc107").text = "Lý ThuyẾT ĐiỆN Cơ BẢN";
    document.getElementById("wdlc108").text = "Đào tạo bên ngoài về kỹ thuật số";
	
		document.getElementById("wdlc109").text = "Strive";

    
});